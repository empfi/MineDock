...
async fn read_packet(stream: &mut TcpStream) -> Result<Vec<u8>, String> {
    let length =
        usize::try_from(read_varint(stream).await?).map_err(|_| "Invalid packet length")?;
    if length > 4096 {
        return Err("Minecraft packet too large".into());
    }
...
async fn client_session(relay: String, token: String, port: u16) -> Result<(), String> {
    ...
    loop {
        ...
        let Some(session) = line.trim().strip_prefix("OPEN ") else {
            continue;
        };
        ...
        data_sessions.spawn(async move {
            let Ok(stream) = TcpStream::connect(&addrs[..]).await else {
                return;
            };
            let _ = stream.set_nodelay(true);
            let Ok(mut remote) = handshake(stream, &token, &format!("DATA {port} {session}")).await
            else {
                return;
            };
...
pub fn start_client(app: AppHandle) {
    tauri::async_runtime::spawn(async move {
        let sessions = Arc::new(Mutex::new(
            HashMap::<u16, tokio::task::JoinHandle<()>>::new(),
        ));
        ...
        loop {
            ...
            let Some((settings, servers)) = config else {
                continue;
            };
            let mut active = sessions.lock().await;
            if !settings.tunnel_enabled
                || settings.tunnel_relay.is_empty()
                || settings.tunnel_token.is_empty()
            {
            ...
            let mut wanted: Vec<u16> = servers
                .into_iter()
...
mod tests {
    ...
    fn signatures_are_stable_and_message_bound() {
        assert_eq!(
            signature("token", "nonce:CONTROL 25565"),
            signature("token", "nonce:CONTROL 25565")
        );
